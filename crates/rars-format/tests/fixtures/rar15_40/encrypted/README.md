...
| `rar4_mixed_visible_names_unknown_password.rar` | unknown | spec repo `fixtures/1.5-4.x/third_party/node-unrar-js/file_enc_by_name_unknown_password.rar` | Mixed RAR4 fixture with visible names, one unencrypted stored member (`1File`), and two encrypted compressed members with unknown passwords. Use only for metadata, stored-member, and wrong-password/NeedPassword behaviour; not as an encrypted-payload success oracle. |
