...
- Improve the Unpack29 writer beyond the current bounded hash-chain baseline:
  ...
  escape 5 offset-one repeats, and PPMd escape 4 distance matches. Embedded
  RARVM records and quality tuning remain open. RAR29 auto-filter placement
  tests whole-member filters, dense opcode clusters, and wider x86 code-section
  spans; RAR29 writers already fall back to store when every encoded candidate
  is larger, and solid archives reset the solid run around stored incompressible
  members.
